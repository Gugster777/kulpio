...
function checkRate(request) {
  ...
  const now = Date.now();
  if (rate.size > 2000) {
    for (const [key, row] of rate) if (now - row.ts >= WINDOW_MS) rate.delete(key);
  }
...
async function handleTranslation(request, env, body, cors) {
  ...
  let texts = Array.isArray(input.texts)
    ? input.texts.map(text => String(text ?? '').slice(0, MAX_TEXT_LENGTH))
    : [];
  ...
  if (!raw) {
    try { raw = await translateWithCloudflare(env, prompt, maxTokens); } catch {}
  }

  const parsed = parseModelJson(raw);
  const out = parsed?.texts;
  if (!Array.isArray(out) || out.length !== texts.length || out.some(text => typeof text !== 'string')) {
    return json({ error: 'translation failed' }, 502, cors);
  }
