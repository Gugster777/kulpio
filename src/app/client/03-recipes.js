...
async function searchRecipes(ev) {
  ...
  const liveQuery = (document.getElementById('recipeSearch')?.value || '').trim();
  if (seq !== _recipeSearchSeq || currentLang !== langAtRequest || currentTab !== 'recipes' || recipesView !== 'sugg' || liveQuery !== q) return;
  const have = fridgeIngredientsEN();
...
async function surpriseRecipe() {
  const seq = ++_recipeSearchSeq;   // invalidate an older cookbook search
  const langAtRequest = currentLang;
  const list = document.getElementById('productList');
  ...
  if (seq !== _recipeSearchSeq || currentLang !== langAtRequest || currentTab !== 'recipes' || recipesView !== 'sugg') return;
