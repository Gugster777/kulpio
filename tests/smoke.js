...
(async () => {
  ...
  // ── Discover tab (replaced the Savings tab) ──
  check('discover tab renders its community shell', await page.evaluate(() => {
    switchTab('deals', document.getElementById('tab-deals'));
    const html = document.getElementById('productList').innerHTML;
    // Offline in the test, so the async fill lands on the offline state — but
    // the shell (kicker + #discList container) is painted synchronously.
    return html.includes('id="discList"') && html.includes('disc-');
  }));
